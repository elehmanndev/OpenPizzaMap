...
const AVPN_FILE = path.join(PATHS.scrapes, 'avpn-scrape.json');
const CACHE_FILE = path.join(PATHS.cache, 'geocode-cache.json');
...
async function passAvpnRescrape() {
  ...
  if (recovered > 0) {
    ...
    const r2 = spawnSync(process.execPath, [path.join(ROOT, 'scripts/backfills/seed-styles.js')], { stdio: 'inherit' });
    if (r2.status !== 0) console.warn('[avpn] seed-styles exited non-zero');

    const r3 = spawnSync(process.execPath, [path.join(ROOT, 'scripts/backfills/flip-avpn-visible.js')], { stdio: 'inherit' });
...
async function passGeocode() {
  ...
  const { spawnSync } = require('child_process');
  const r = spawnSync(process.execPath, [path.join(ROOT, 'scripts/importers/import-places.js')], { stdio: 'inherit' });
