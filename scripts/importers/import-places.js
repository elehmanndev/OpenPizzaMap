...
const CACHE_FILE = path.join(ROOT, '.cache', 'geocode-cache.json');
const ERRORS_FILE = path.join(ROOT, '.cache', 'import-errors.json');
...
// ----- collect + dedupe -----
// Scrape JSONs live under data/scrapes/ since the 2026-05-05 cleanup.
// Older runs left them at the repo root; treat root as a fallback so any
// in-flight tarball still works.
function _resolveScrapeFile(file) {
  const inData = path.join(ROOT, 'data', 'scrapes', file);
...
function loadAll() {
  ...
  for (const cfg of SOURCES) {
    const full = _resolveScrapeFile(cfg.file);
