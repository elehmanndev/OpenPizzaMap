...
// =============================================================================
// DEPRECATED — DO NOT USE FOR NEW WORK (quarantined 2026-05-05)
// =============================================================================
//
// This is the original 6-phase enrichment workhorse. Its job is now done by
// the proper pipeline at src/services/enrichment/, called from:
//   - scripts/importers/import-places.js (every new import goes through it)
//   - scripts/enrichment/batch-enrich.js (backfill — runs nightly via cron)
//   - src/routes/api.submissions.js (user-submitted places)
//
// The new pipeline uses Google Places API (with Playwright fallback) and a
// proper EnrichmentCache table; this script uses ad-hoc DDG searches + per-
// script JSON caches and is not aware of the new googlePlaceId column.
// Running it on rows that have already been through the new pipeline will
// re-process them with worse data quality.
//
// If you find yourself reaching for this file: STOP. Use batch-enrich.js
// instead. Set LEGACY_ENRICHER_FORCE=1 to override the guard below if you
// really know what you're doing (you almost never do).
// =============================================================================

if (!process.env.LEGACY_ENRICHER_FORCE) {
  console.error("================================================================");
  console.error(" DEPRECATED — scripts/legacy/enricher.js is no longer supported.");
  console.error(" Use scripts/enrichment/batch-enrich.js instead.");
  console.error(" Set LEGACY_ENRICHER_FORCE=1 to override (you almost never want to).");
  console.error("================================================================");
  process.exit(1);
}
